...
ИмяФайлаВызова                          =   "v8confbuilder.os";
ПолныйПутьФайлаВызова                   =   СтартовыйСценарий().Каталог + "/" + ИмяФайлаВызова;
...
Тогда
    ...
    Иначе
        ...
        КопироватьФайл(ПолныйПутьФайлаВызова, Путь + "/.git/" + ИмяФайлаВызова);
    КонецЕсли;
	...
	|        ""--rac""              :   ""Полный путь к клиенту rac.exe. Нужно экранировать."",
    |        ""--db-user""          :   ""User"", 
    |        ""--db-pwd""           :   ""Password"", 
    ...
    Сообщить("Теперь настройте конфигурацию в файле " + Путь + "/.git/env.json");
    ЗавершитьРаботу(0);
...
Попытка
	...
	ПутьКRac							=	НастройкиПоУмолчанию.Получить("--rac");
...
Если
	ЗначениеЗаполнено(ПутьКRac)
Тогда
	Команда.ДобавитьПараметр("--rac "   +   ПутьКRac);
КонецЕсли;
